<?php
	...
	class Productos extends CI_Controller
	{
        ...
        public function index()
        {
	        ...
	        $data['producto'] = $this->productoModel->productos(" order by pro.Pro_Oferta desc");
		...
		public function comentario()
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				$tienda = $this->cotizacionModel->get_datos_correo($Tie_IdTienda);
				$producto = $this->productoModel->get_producto_id($Pro_IdProducto);

				if ($registro)
				{
				$this->enviarComentario($tienda->Usu_Correo, $tienda->Per_Nombre,$data['usuario']['nombre'],$producto->Pro_Nombre,$tienda->Tie_Subdominio);
				echo $Pro_IdProducto;			
				}
		...
		public function ajax_favorito()
	    {
	    	if ($this->input->is_ajax_request()) {
		        ...
		        $Pro_IdProducto=(int)$this->input->post('id_producto');

		        if($valor==1) {
		        	$mensaje="Añadido a favorito";
		        	$registro=$this->productoFavoritoModel->registrar($Pro_IdProducto, $data['usuario']['id_usuario']);
		        } else {
		        	$mensaje="Eliminado de favorito";
		        	$registro=$this->productoFavoritoModel->eliminar($Pro_IdProducto, $data['usuario']['id_usuario']);
		        }
	    ...
	    function fb_signin($accessToken)
        {
            ...
            if(!isset($accessToken)) {
              echo 'No cookie set or no OAuth data could be obtained from cookie.';exit;
            }
            ...
            $fb_user=$this->facebookModel->getUser($accessToken);
           	if(isset($fb_user)) {
                ...
                if(count($usuario) > 0) {
                    $this->session->set_userdata(array('id_usuario'=>$usuario->Usu_IdUsuario,'nombre_usuario'=>$Per_Nombre,'rol_usuario'=>$usuario->Rol_IdRol,'tienda_usuario'=>$usuario->Tie_IdTienda,'logueado'=>($usuario->Usu_Activated==1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED));
                    $this->_redirigir_login();
                } else {
                    $Usu_Clave=$this->generate_password(9,8);
                    $Per_IdPersona=NULL;
                    $Per_Dni=NULL;
                    
                    if(!is_null($data=$this->tank_auth->create_comprador('',$Per_IdPersona,$Per_Dni,$Per_Nombre,$Usu_Correo,$Usu_Clave,$email_activation)))
                    {
                        $data['site_name']=$this->config->item('website_name', 'tank_auth');
                        ...
                        $this->_login($Usu_Correo, $Usu_Clave, 0);
                    } else {
                        echo $this->tank_auth->error['username'];
                    }
                }           
            } else { 
