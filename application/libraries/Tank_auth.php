...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) 
		{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) 
			{
				...
				if ($hasher->CheckPassword($password, $user->Usu_Clave)) 
				{
					// password ok
					if ($user->Usu_Banned == 1) 
					{								
						// fail - banned
						$this->error = array('banned' => $user->Usu_BanReason);
					} 
					else 
					{
						$persona = $this->ci->users->get_persona_x_id($user->Per_IdPersona);

						$this->ci->session->set_userdata(array(
							'id_usuario' => $user->Usu_IdUsuario,
							'nombre_usuario' => $persona->Per_Nombre,
							'rol_usuario' => $user->Rol_IdRol,
							'tienda_usuario' => $user->Tie_IdTienda,
							'logueado' => ($user->Usu_Activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
						));

						if ($user->Usu_Activated == 0) 
						{							
						...
						else 
						{
							...
							if ($remember) 
							{
								$this->create_autologin($user->Usu_IdUsuario);
							}
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'logueado' => ''));
	...
	{
		return $this->ci->session->userdata('logueado') === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
	}
	...
	function get_rol()
	{
		return $this->ci->session->userdata('rol_usuario');
	}

	function get_tienda()
	{
		return $this->ci->session->userdata('tienda_usuario');
	}
	 ...
	 * @return	array
	 */
	function create_comprador($username, $Per_IdPersona, $Per_Dni, $Per_Nombre, $Usu_Correo, $Per_Telefono,  $Usu_Clave, $email_activation, $publicidad, $Usu_Flag_Publicidad)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) 
		{
			$this->error = array('username' => 'auth_username_in_use');
		} 
		elseif (!$this->ci->users->is_email_available($Usu_Correo)) 
		{
			$this->error = array('Usu_Correo' => 'auth_email_in_use');
		} 
		else 
		{
			...
			$hashed_password = $hasher->HashPassword($Usu_Clave);

			$Usu_LastIp = $this->ci->input->ip_address();			

			if ($email_activation) 
			{
				$Usu_NewEmailKey = md5(rand().microtime());
			}
			else
			{
				$Usu_NewEmailKey = NULL;
			}
			if (!is_null($Usu_IdUsuario = $this->ci->users->crear_comprador($Per_IdPersona, 1, $Per_Dni, $Per_Nombre, $Usu_Correo, $Per_Telefono, $hashed_password, $Usu_LastIp, $Usu_NewEmailKey, !$email_activation, $publicidad, $Usu_Flag_Publicidad))) 
			{
				$data['Usu_IdUsuario'] = $Usu_IdUsuario;
	...
	function create_comprador2($username, $Per_IdPersona, $Per_Dni, $Per_Nombre, $Usu_Correo, $Per_Telefono,  $Usu_Clave, $email_activation, $publicidad, $Usu_Flag_Publicidad,$vendedor)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) 
		{
			$this->error = array('username' => 'auth_username_in_use');
		...
		else 
		{
			...
			if (!is_null($Usu_IdUsuario = $this->ci->users->crear_comprador($Per_IdPersona, 1, $Per_Dni, $Per_Nombre, $Usu_Correo, $Per_Telefono, $hashed_password, $Usu_LastIp, $Usu_NewEmailKey, !$email_activation, $publicidad, $Usu_Flag_Publicidad,$vendedor))) 
			{
				$data['Usu_IdUsuario'] = $Usu_IdUsuario;
				$data['Usu_Clave'] = $Usu_Clave;
	...
	function create_tienda($username, $Per_IdPersona, $Per_Dni, $Per_Nombre, $Per_Telefono, $Usu_Correo, $Tie_ruc_dni, $Tie_Tipo, $Tie_Nombre, $Tie_RazonSocial, $Tie_Subdominio, $Tie_FechaInscripcion, $Pla_IdPlan, $Usu_Clave, $email_activation, $Codigo)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username))
		{
			$this->error = array('username' => 'auth_username_in_use');
		...
		else 
		{
			...
			if (!is_null($Usu_IdUsuario = $this->ci->users->crear_tienda($Per_IdPersona, 1, $Per_Dni, $Per_Nombre, $Per_Telefono, $Usu_Correo, $Tie_ruc_dni, $Tie_Tipo, $Tie_Nombre, $Tie_RazonSocial, $Tie_Subdominio, $Tie_FechaInscripcion, $Pla_IdPlan, $hashed_password, $Usu_LastIp, $Usu_NewEmailKey, !$email_activation, $Codigo)))
			{
				$data['Usu_IdUsuario'] = $Usu_IdUsuario;
				$data['Usu_Clave'] = $Usu_Clave;
	...
	function create_usuario($username, $Per_IdPersona, $Per_Dni, $Per_Nombre, $Per_Celular, $Usu_Correo, $Usu_Clave, $Rol_IdRol, $Tie_IdTienda, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) 
		{
			$this->error = array('username' => 'auth_username_in_use');
		...
		else 
		{
			...
			if (!is_null($Usu_IdUsuario = $this->ci->users->crear_usuario($Per_IdPersona, 1, $Per_Dni, $Per_Nombre, $Per_Celular, $Usu_Correo, $hashed_password, $Usu_NewEmailKey, $Rol_IdRol, $Tie_IdTienda, $Usu_LastIp, !$email_activation))) 
			{
				$data['Usu_IdUsuario'] = $Usu_IdUsuario;
				$data['Usu_Clave'] = $Usu_Clave;
	...
	function forgot_password($login)
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_login($login))) {
				...
				$this->ci->users->set_password_key($user->Usu_IdUsuario, $data['new_pass_key']);
				return $data;
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE)))
		{
			...
			if ($hasher->CheckPassword($old_pass, $user->Usu_Clave)) 
			{
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					if (!is_null($user = $this->ci->user_autologin->get($data['user_id'], md5($data['key'])))) {
						...
						$this->ci->session->set_userdata(array(
								'id_usuario'	=> $user->Usu_IdUsuario,
								'nombre_usuario' => $user->Usu_Correo,
								'logueado'	=> STATUS_ACTIVATED,
						));
