<?php 
    ...
    class Producto_model extends CI_Model
    {
       ...
       function vendedores() {            
        ...
        {
            $this->db->select('pro.Pro_IdProducto, pro.Pro_Nombre, pro.Pro_Descripcion, pro.Pro_FechaModificacion, pro.Pro_FechaCreacion, pro.Pro_PrecioMaximo, pro.Pro_PrecioMinimo, pro.Pro_PM,(SELECT Prf_Thumb FROM producto_foto as prf WHERE prf.producto_Pro_IdProducto = pro.Pro_IdProducto LIMIT 1) as Prf_Thumb, (SELECT SUM(SKU_StockDisponible) FROM sku as sku WHERE sku.producto_Pro_IdProducto = pro.Pro_IdProducto LIMIT 1) as SKU_StockDisponible,(SELECT SKU_IdSKU FROM sku as sku WHERE sku.producto_Pro_IdProducto = pro.Pro_IdProducto LIMIT 1) as SKU_IdSKU, pro.Pro_Saldo, pro.Pro_Preventa, (SELECT cat.Cat_IdCategoria FROM categoriap as cat INNER JOIN producto_categoria as prc ON prc.Cat_IdCategoria = cat.Cat_IdCategoria WHERE prc.producto_Pro_IdProducto = pro.Pro_IdProducto LIMIT 1) as idcategoria');
            $this->db->from('producto as pro');
